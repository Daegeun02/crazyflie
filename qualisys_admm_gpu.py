...
def ker_calnext():
    ...
    __device__ void matmul_(float* x, float*y, int first, int second, int third, float* matmul_result, int tr_tfs){
    ...
    float* temp_sum, float* z_buffer, float* residuals, int* optimality){
        ...
        for (int j = 0; j < k ; j++){
            ...
            matmul_(A, temp_sum, len_z, 6, 1, temp, 1);
            temp_sum[idx] = -rho1*temp[idx] - rho2*(-z[idx]+u2[idx]);
            //__syncthreads();
            sync(grid);

            matmul_(coeff_x, temp_sum, len_z, len_z, 1, x, 0);
            z[idx] = x[idx] + u2[idx];
...
def ker_make_matrix():
    ...
    extern "C" __global__ void make_matrix(float dt, float* result_G, \
    int N, float* result_c, float* x_0, float* x_des)
    {
...
optimizer_gpu.ADMM_opt_gpu(60,30)

print(mempool.used_bytes()) # 0
print(mempool.total_bytes()) # 512

admm_u = cp.asnumpy(optimizer_gpu.x_gpu)
admm_u = admm_u.reshape(n,3)
